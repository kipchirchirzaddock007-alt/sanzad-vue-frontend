...
<template>
  <section class="page">
    <h2>Media Feed</h2>
    <p>
      Public feed of citizen reports and a full catalogue of projects with on-the-ground photos and videos from locals.
    </p>
    ...
    <div class="layout">
      ...
      <div class="card">
        ...
        <div class="list">
          <p v-if="loading && !reports.length">Loading…</p>
          <p v-else-if="!reports.length">No reports yet.</p>
          ...
          <article
            ...
            class="report"
          >
            <h3>{{ r.issueType }} – {{ r.location }}</h3>
            <p class="meta">
              Reporter: {{ r.reporterName }} · Status: {{ r.status }} · Created: {{ r.createdAt }}
            </p>
            <p>{{ r.description }}</p>
            <p v-if="r.leaderNote">
              Leader note: {{ r.leaderNote }}
            </p>
